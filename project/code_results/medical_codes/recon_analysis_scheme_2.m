...
% x_recon = reshape(uint8(normalize(abs(x_recon), 'norm') * 255), 109, 109);
% img2_recon_2 = idct2(x_recon);
% img2_recon_2 = uint8(img2_recon_2);
% imshow(x_recon);

function ret = phi_mat(x)
n = 109;
% theta = 0:10:170;
x_mat = reshape(x, n, n);
% x_radon = radon(x_mat, theta);
y_mat = fft2(x_mat, n, n);
ret = reshape(y_mat, [], 1);
end

function ret = phit_mat(x)
n = 109;
% theta = 0:10:170;
x_mat = reshape(x, n, n);
x_ifft = ifft2(x_mat, n, n);
% x_iradon = iradon(x_ifft, theta, "linear", "Ram-Lak", 1, n);
ret = reshape(x_ifft, [], 1);
end
